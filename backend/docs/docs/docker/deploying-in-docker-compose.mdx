...
The `expose: 5980` is what allows the `rasa` service to reach the `app` service on that port.
...
You should then be able to interact with your bot via requests to port 5980, on the webhook endpoint that
corresponds to a [configured channel](#configuring-channels):

```bash
curl -XPOST http://localhost:5980/webhooks/rest/webhook \
