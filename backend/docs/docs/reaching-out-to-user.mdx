...
Finally, add a template for the `utter_welcome` response to your domain:
...
curl -H "Content-Type: application/json" -X POST \
  ...
  "http://localhost:5980/conversations/user123/trigger_intent?output_channel=latest"
```
...
"http://localhost:5980/conversations/user1234/trigger_intent?output_channel=latest"
```
